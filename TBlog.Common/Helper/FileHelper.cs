...
namespace TBlog.Common
{
    public class FileHelper : IDisposable
    {
        ...
        public static string GetAvailableFileWithPrefixOrderSize(string folderPath, string prefix, int size = 1 * 1024 * 1024, string ext = ".log")
        {
            ...
            return Path.Combine(folderPath, $@"{prefix}_{DateTime.Now.ToDateTimeStamp()}.log");
        }
        #endregion
        ...
        {
            if (encode == null) encode = Encoding.GetEncoding("gb2312");
        ...
        public static (bool,string) IsBase64String(string base64)
        {
            Regex DataUriPattern = new Regex(@"^data\:(?<mimeType>image\/(?<imageType>png|tiff|jpg|gif));base64,(?<data>[A-Z0-9\+\/\=]+)$",
                RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
            if (string.IsNullOrWhiteSpace(base64)) return (false, string.Empty);

            Match match = DataUriPattern.Match(base64);
